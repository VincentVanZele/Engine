...
proxy::TextObject::TextObject(const std::string& text, std::shared_ptr<Font> font) 
	: mNeedsUpdate(true), mText(text), mFont(font), mTexture(nullptr)
{ }

void proxy::TextObject::Update()
{
	if (mNeedsUpdate)
	{
...
void proxy::TextObject::Render() const
{
	if (mTexture != nullptr)
	{
...
void proxy::TextObject::SetText(const std::string& text)
{
	mText = text;
	mNeedsUpdate = true;
}

void proxy::TextObject::SetPosition(const float x, const float y)
{
